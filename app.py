...
# パスワードチェック (デプロイ環境でのみ有効化)
# ローカルテスト等でsecretsがない場合のエラー回避
...
def solve_schedule(df_req, df_teacher, df_const, weights, recalc_classes, manual_fixes):
    ...
    # 📌 手動固定の適用 (簡易版)
    # manual_fixes は {'t': '田中', 'd': '月', 'p': 1, 'val': '11'}
    if manual_fixes:
        for fix in manual_fixes:
            ...
            if (t_name, d, p) in teacher_vars:
                # この時間に何かの授業が入ることを強制
                # (どのクラスかを特定するのはDataEditorの戻り値だけでは難しいため、
                #  「この先生はこの時間は埋まる」という固定にする)
                model.Add(sum(teacher_vars[(t_name, d, p)]) == 1)
...
if f_req and f_teacher and f_const:
    ...
    if 'schedule_df' not in st.session_state:
        st.info("👈 サイドバーでCSVをアップロードし、「作成開始」ボタンを押してください。")
    else:
        st.subheader("📅 時間割プレビュー (直接編集モード)")
        st.markdown("セルを書き換えて「作成開始」を押すと、その内容が**固定**されて再計算されます。")
        ...
        for _, cr in df_const.iterrows():
            ...
            if t in teachers:
                ...
                # セルが空なら入れる
                current_val = df_view.loc[mask, t].values[0]
                if not current_val:
        ...
        # 差分検知 (固定用)
        # 編集されたセルを特定し、manual_fixes リストを作成する
        manual_fixes = []
    ...
    if st.sidebar.button("🚀 作成開始 (または再計算)"):
        ...
        if 'schedule_df' in st.session_state:
            # edited_df から固定リストを作る
            # (全スキャンは重いが、Streamlit Cloudなら許容範囲)
            # ここでは「値が入っているセルはすべて固定候補」とするが、
            # 再計算クラスの枠は除外するなど、運用で工夫が必要
            pass 
